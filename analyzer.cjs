...
const hints = {
  ...
  strictmode: "React.StrictMode ada di dev — tidak di production. Di dev gunakan guard jika efek sensitif terhadap double-run.",
    ...
    enter(pathNode) {
      ...
      if (node.type === "CallExpression" &&
        ...
        if (!depArray) {
          ...
          deps.forEach((d) => {
            if (!d) return;
            // if dependency contains Identifier which is a function name (heuristic: name contains handle/update/on/dispatch)
            if (d.type === "Identifier") {
              const name = d.name || "";
              if (/^(handle|on|update|set|dispatch|fetch|get|load)/i.test(name)) {
                report.findings.push(makeFinding({
                  file,
                  message: `useEffect memiliki dependency fungsi atau handler '${name}' — fungsi yang tidak termemoisasi dapat menyebabkan infinite loop.`,
                  kind: "dependency_fn",
                  loc: node.loc
                }));
              }
            }
            // if dependency is MemberExpression or CallExpression, flag it
            if (d.type === "MemberExpression" || d.type === "CallExpression") {
              report.findings.push(makeFinding({
                file,
                message: `useEffect memiliki dependency non-primitif (MemberExpression/CallExpression) → dapat menyebabkan rerun tak terduga.`,
                kind: "dependency_fn",
                loc: node.loc
              }));
            }
          ...
          // scan effect function body for setState/updateAssetData etc
          if (effectFn && (effectFn.type === "ArrowFunctionExpression" || effectFn.type === "FunctionExpression")) {
            ...
            if (/updateAssetData|set[A-Z0-9_]+|dispatch|setState|setAssets|setRisks/.test(bodyStr)) {
              // if dep array contains functions or context values, high risk
              if (deps.some(d => d && (d.type === "Identifier" || d.type === "MemberExpression"))) {
                report.findings.push(makeFinding({
                  file,
                  message: "Effect memanggil updater (setSomething/updateAssetData) dan dependency mencakup fungsi/objek → risiko loop/refetch.",
                  kind: "infinite_effect",
                  loc: node.loc
                }));
              }
